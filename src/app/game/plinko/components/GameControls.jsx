...
export default function GameControls({ onBet, onRowChange, onRiskLevelChange, onBetAmountChange, initialRows = 16, initialRiskLevel = "Medium" }) {
  ...
  const handleBet = () => {
    ...
    if (betValue < 0.001) {
      alert("Minimum bet amount is 0.001 MOCA");
      return;
    }
    
    if (betValue > currentBalance) {
      alert(`Insufficient balance! You have ${currentBalance.toFixed(5)} MOCA but need ${betValue} MOCA`);
      return;
    }
  ...
  const startAutoBetting = () => {
    ...
    if (totalBetAmount > currentBalance) {
      alert(`Insufficient balance for ${totalBets} bets of ${betAmount} MOCA each. You need ${totalBetAmount.toFixed(3)} MOCA but have ${currentBalance.toFixed(5)} MOCA`);
      setIsAutoPlaying(false);
      return;
    }
  ...
  return (
    <div className="bg-[#1A0015] rounded-xl border border-[#333947] p-6">
      ...
      <div className="mb-6">
        ...
        <div className="mb-2">
          <span className="text-2xl font-bold text-white">{betAmount} MOCA</span>
        </div>
      ...
      {gameMode === "auto" && isAutoPlaying ? (
        ...
        <div className="space-y-3">
          ...
          <div className="text-center p-3 bg-[#2A0025] rounded-lg border border-[#333947]">
            ...
            {isConnected ? (
              <div className="text-lg font-bold text-green-400">{getCurrentBalanceInMON()} MOCA</div>
            ) : (
