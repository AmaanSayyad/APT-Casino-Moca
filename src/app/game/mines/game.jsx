...
const Game = ({ betSettings = {}, onGameStatusChange, onGameComplete }) => {
  ...
  const defaultSettings = {
    betAmount: 0.001, // Default to 0.001 MOCAA
    mines: 5,
    isAutoBetting: false,
    tilesToReveal: 5,
  };
  ...
  useEffect(() => {
    ...
    if (Object.keys(betSettings).length > 0) {
      ...
      const startGameWithBet = async () => {
       ...
       // Check Redux balance MOCA)A)
        const currentBalance = parseFloat(userBalance || '0');
        
        if (currentBalance < parseFloat(settings.betAmount)) {
          toast.error(`Insufficient balance. You have ${currentBalance.toFixed(5)} MOCA but need ${parseFloat(settings.betAmount)} MOCA`);
          return;
        }
        ...
        try {
          ...
          toast.success(`Bet placed! ${parseFloat(settings.betAmount).toFixed(5)} MOCA deducted from balance`);
          ...
          if (settings.isAutoBetting && settings.aiAssist) {
            ...
            toast.info(`Bet placed: ${parseFloat(settings.betAmount).toFixed(5)} MOCA, ${settings.mines} mines`);
          }
  ...
  const cashout = () => {
    ...
    try {
      ...
      // The actual payout was already handled in the initial bet transaction
              toast.success(`Cashed out: ${payout.toFixed(5)} MOCA (${multiplier.toFixed(2)}x)`);
      playSound('cashout');
        ...
        {hasPlacedBet && isPlaying && !gameOver && !gameWon && (
          <div className="flex gap-3">
            ...
            >
              ...
              <span>CASH OUT ({calculatePayout()} MOCA)</span>
            </button>
          </div>
        )}
        ...
        {gameWon && (
          <div className="text-center py-3 bg-gradient-to-r from-green-600 to-emerald-600 rounded-lg text-white font-bold">
            ...
            <div className="mt-2 text-sm opacity-90">
              Winnings: {calculatePayout()} MOCA ({multiplier.toFixed(2)}x)
            </div>
          </div>
        )}
