...
const WithdrawModal = ({ isOpen, onClose }) => {
  ...
  // Display balance MOCA formatt
  const balanceInApt = parseFloat(userBalance || '0') / 100000000;
  const maxWithdraw = Math.max(0, balanceInApt - 0.01);// Reserve 0.01 MOCA for gas feeses
  ...
  const validateWithdraw = () => {
    ...
    if (amount > maxWithdraw) {
      setError(`Insufficient balance. Max withdraw: ${maxWithdraw.toFixed(4)} MOCA`);
      return false;
    }
    
    if (amount < 0.001) {
      setError('Minimum withdraw amount is 0.001 MOCA');
      return false;
    }
  ...
  const confirmWithdraw = async () => {
    ...
    try {
      ...
      setStep('success');
      toast.success(`Successfully withdrew ${amount} MOCA! TX: ${result.transactionHash.slice(0, 8)}...`);
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Withdraw MOCA</h3>
              <p className="text-gray-400">Transfer your winnings to your Moca Testnet wallet</p>
            </div>
            
            <div className="bg-gray-800/50 rounded-lg p-4">
              <div className="flex justify-between items-center mb-2">
                <span className="text-gray-400">Available Balance:</span>
                <span className="text-white font-bold">{balanceInApt.toFixed(4)} MOCA</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-400">Max Withdraw:</span>
                <span className="text-green-400 font-bold">{maxWithdraw.toFixed(4)} MOCA</span>
              </div>
            </div>
            
            <div>
              <label className="block text-gray-300 mb-2">Withdraw MOCA)</label>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-800/50 rounded-lg p-4 space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-white font-bold">{withdrawAmount} MOCA</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-yellow-400">~0.001 MOCA</span>
              </div>
              <hr className="border-gray-600" />
              <div className="flex justify-between">
                <span className="text-gray-400">You'll Receive:</span>
                <span className="text-green-400 font-bold">{(parseFloat(withdrawAmount) - 0.001).toFixed(4)} MOCA</span>
              </div>
            </div>
      ...
      case 'success':
        return (
          <div className="space-y-6 text-center">
            ...
            <p className="text-gray-400">Your MOCA has been sent to your wallet</p>
            
            <div className="bg-green-900/20 border border-green-500/30 rounded-lg p-4">
              <div className="flex justify-between mb-2">
                <span className="text-gray-400">Amount Sent:</span>
                <span className="text-green-400 font-bold">{withdrawAmount} MOCA</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">To Wallet:</span>
                <span className="text-white font-mono text-sm">{account?.address?.slice(0, 6)}...{account?.address?.slice(-4)}</span>
